...
class PakBus(object):
    ...
    def decode_packet(self, data):
        ...
        try:
            ...
            raise ValueError(f'Could not unpack {data[0:8]} as >4H')
    ...
    def get_values_cmd(self, table_name: str, field_name: str, Swath=1, SecurityCode=0x0000):
        ...
        transac_id = self.transaction.next_id()  # Generate new transaction number
        hdr = self.pack_header(0x1, 0x2, self.RING)
        msg = self.encode_bin(['Byte', 'Byte', 'UInt2', 'ASCIIZ', 'Byte', 'ASCIIZ', 'UInt2'],
                              [0x1a, transac_id, SecurityCode, table_name, 16, field_name, Swath])
        return b''.join((hdr, msg)), transac_id

    def set_values_cmd(self, table_name: str, field_name: str, value: float, Swath=1, SecurityCode=0x0000):
        # table_name:    Table name as string
        # field_name:    Field name (including index if applicable)
        ...
        hdr = self.pack_header(0x1, 0x2, self.RING)
        type_code = 'IEEE4B'
        code_enc = self.DATATYPE[type_code]['code']
        msg = self.encode_bin(
                        ['Byte', 'Byte',    'UInt2',       'ASCIIZ',  'Byte',    'ASCIIZ', 'UInt2',  type_code],
                        [0x1b, transac_id, SecurityCode, table_name, code_enc,  field_name, Swath,     value])
        return b''.join((hdr, msg)), transac_id
